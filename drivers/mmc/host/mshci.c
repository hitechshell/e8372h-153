...
static void mshci_reset_fifo(struct mshci_host *host)
{
	...
	while (mshci_readl(host, MSHCI_CTRL) & FIFO_RESET) {
		if (timeout == 0) {
			DBG(" Reset FIFO never completed.\n");
			mshci_dumpregs(host);
			return;
		}
		timeout--;
...
static void mshci_init(struct mshci_host *host)
{
    ...
    if (host->mmc->caps & MMC_CAP_SDIO_IRQ) /* wifi */
    {
...
static void mshci_clock_onoff(struct mshci_host *host, bool val)
{
	...
	if (reg & INTMSK_HLE) {
		DBG(" HLE in Clock %s. RINTSTS = 0x%x\n ", val ? "ON" : "OFF", reg);
		mshci_writel(host, 0, MSHCI_CMD);
		mshci_writel(host,INTMSK_HLE,MSHCI_RINTSTS);
	} else if (loop_count == 0) {
		DBG(" Clock %s has been failed.\n ",	 val ? "ON" : "OFF");
	}
...
static void mshci_send_command(struct mshci_host *host, struct mmc_command *cmd)
{
	...
	if (cmd->opcode == SD_SWITCH_VOLTAGE) {
        ...
        mshci_writel(host, 0x02, MSHCI_ENABLE_SHIFT);/* xuchao: fix hisi bug,for lexar 64G card */
		loop_count = 0x100000;
		do {
			if (!(mshci_readl(host, MSHCI_CMD) & CMD_STRT_BIT))
				break;
...
static int mshci_get_cd(struct mmc_host *mmc)
{
	...
	if (host->quirks & MSHCI_QUIRK_WLAN_DETECTION) {
		cdetect = host->flags & MSHCI_DEVICE_DEAD ? 0 : 1;
	} else if (host->quirks & MSHCI_QUIRK_EXTERNAL_CARD_DETECTION) {
		cdetect = host->ops->get_present_status(host);
	} else {
		cdetect = !mshci_readl(host, MSHCI_CDETECT);
		DBG("data3 cdetect: %u\n", cdetect);
	}
...
int mshci_execute_tuning(struct mmc_host *host, u32 opcode)
{
	...
	if(ms_host->ops && ms_host->ops->tuning_find_condition) {
		if(host->ios.timing == MMC_TIMING_UHS_SDR104) {
...
static irqreturn_t mshci_irq(int irq, void *dev_id)
{
	...
	if (host->cmd && (host->cmd->opcode == SD_SWITCH_VOLTAGE) 
        ...
        mshci_writel(host, 0x00, MSHCI_ENABLE_SHIFT);/* xuchao: fix hisi bug,for lexar 64G card */
		mshci_finish_command(host);
		intmask &= ~INTMSK_VOLT_SWITCH; 
	}
...
static void mshci_resume_init(struct mshci_host *host)
{
	...
	/*
	  enable CARD DETECT interrupt again after system resume from deep sleep, or sd card insert
...
int mshci_add_host(struct mshci_host *host)
{
	...
	ret  = device_create_file(mmc->parent, &dev_attr_debug);
	ret |= device_create_file(mmc->parent, &dev_attr_quirks);
