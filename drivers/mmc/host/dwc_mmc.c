...
#include <product_config.h>
    ...
    #include "../../wlan_if/wlan_if.h"
    #if defined(BSP_CONFIG_BOARD_E5773S_156) || defined(BSP_CONFIG_BOARD_E5_E5770s) || defined(BSP_CONFIG_BOARD_E5771S_852) \
        || defined(BSP_CONFIG_BOARD_E5771H_937) || defined(BSP_CONFIG_BOARD_E5573CS) || defined(BSP_CONFIG_BOARD_E5573BS)\
        || defined(BSP_CONFIG_BOARD_E5573CS_933) || defined(BSP_CONFIG_BOARD_607HW) || defined(BSP_CONFIG_BOARD_E5577BS_937) \
        || defined(BSP_CONFIG_BOARD_R218h)
		#define BSP_WIFI_VCC33_ENABLE FEATURE_OFF
	#else
		#define BSP_WIFI_VCC33_ENABLE FEATURE_ON
	#endif
#else
    #define CONFIG_WIFI_SDIO  FEATURE_OFF
#endif
...
#if (FEATURE_ON == MBB_COMMON) && (FEATURE_ON == BSP_WIFI_VCC33_ENABLE)
...
static int __devinit hi_mci_probe(struct platform_device *pdev)
{
...
#if (FEATURE_ON == MBB_COMMON)
    /* must have platform data , so should be protected*/
    WARN_ON(!pdev);
    if (!pdev)
    {
        return -ENODEV;
    ...
    if (NULL != ms_host) 
    {
        dev_warn(&pdev->dev, "%s mmc dev have existed!\n", __func__);
        return 0;
    }
#endif
...
static int __devexit hi_mci_remove(struct platform_device *pdev)
{
	...
	if (!ms_host) {
    #if (FEATURE_ON == MBB_COMMON)
        dev_warn(&pdev->dev, "%s mmc dev have been removed!\n", __func__);
        return 0;
    #else
        return -ENODEV;
    #endif
...
static int hi_mci_resume(struct device *dev)
{
    ...
    msleep(2);   // 在sd vcc上电后延时2ms，规避解决sd卡在休眠唤醒时概率性初始化异常问题
	if (0 == hi_host->pdev->id) {	
		gpio_to_mmc0_mux();
		g_hi_mci_debug_state.gpio_to_mmc++;
	}
...
{   
...
#if (FEATURE_ON == BSP_WIFI_VCC33_ENABLE)
    /* 3.3v电源 */
    ...
    BSP_BUK3_ENABLE(BUK3_MOD_WiFi, 1);
...
int mshci_wifi_power_off(void)
{
    ...
    BSP_BUK3_ENABLE(BUK3_MOD_WiFi, 0);
...
static int __init hi_mci_init(void)
{
    ...
    (void)mshci_wifi_power_off();
    msleep(200); /*睡眠200毫秒，保证下电*/
