...
{
	int ret;
	ret = mmc_app_cmd(test->card->host, test->card);
	if (ret)
		return ret;	
...
static int mmc_test_random_perf(struct mmc_test_card *test, 
	int write, int perf, unsigned long *total_size)
{
	struct mmc_test_area *t = &test->area;
	unsigned int next;
	unsigned long sz;
	...
	for (sz = 512; sz < t->max_tfr; sz <<= 1) {
		...
		ret = mmc_test_rnd_perf(test, write, perf, sz);
		if (ret)
			return ret;
		TOTAL_SIZE(total_size, sz);
	}
 ...
 */
static unsigned long g_read_total;

static int mmc_test_random_read_perf(struct mmc_test_card *test)
{
	return mmc_test_random_perf(test, 0, 1, &g_read_total);
}

/*
 * Random write performance by transfer size.
 */
static unsigned long g_write_total;

static int mmc_test_random_write_perf(struct mmc_test_card *test)
{
	return mmc_test_random_perf(test, 1, 1, &g_write_total);
}
void mmc_test_stress_count(void)
{
	pr_info("Stress read %lu\n", g_read_total);
	pr_info("Stress write %lu\n", g_write_total);
}
...
/*
 * Stress performance by transfer size.
 */

static int mmc_test_stress_random_read_perf(void * card)
{
	int err;
	struct mmc_test_card *test = (struct mmc_test_card *)card;
	err = 0;
	pr_info("mmc test read thread starting\n");
	while(!err) {
			mmc_claim_host(test->card->host);
		 	err =mmc_test_random_perf(test, 0, 0, &g_read_total);
			if (err) {
				pr_info("mmc test read thread exiting, times(%lu)\n", g_read_total);
			}
...
static int mmc_test_stress_random_write_perf(void * card)
{
	int err;
	struct mmc_test_card *test = (struct mmc_test_card *)card;
	err = 0;
	pr_info("mmc test write thread starting\n");
	while(!err) {
			mmc_claim_host(test->card->host);
		 	err = mmc_test_random_perf(test, 1, 0, &g_write_total);
			if (err) {
				pr_info("mmc test write thread exiting, times(%lu)\n", g_read_total);
			}
			mmc_release_host(test->card->host);
		}
	return err;
}
static int mmc_test_stress_perf(struct mmc_test_card *test)
{
	struct mmc_test_card *test_stress;
	rnd_turn = 2;
	test_stress = kzalloc(sizeof(struct mmc_test_card), GFP_KERNEL);
	if (!test_stress)
		return -ENOMEM;
	memcpy(test_stress, test, sizeof(struct mmc_test_card));
	
	test_stress->card = kzalloc(sizeof(struct mmc_card), GFP_KERNEL);
	if (!test_stress->card)
		return -ENOMEM;
	memcpy(test_stress->card, test->card, sizeof(struct mmc_card));

	read_task = kthread_create(mmc_test_stress_random_read_perf, 
		test_stress, "mmc_stress_read");
	write_task = kthread_create(mmc_test_stress_random_write_perf, 
		test_stress, "mmc_stress_write");
...
static const struct mmc_test_case mmc_test_cases[] = {
	...
	{
		.name = "Stress performance by transfer size",
		.prepare = mmc_test_area_prepare,
		.run = mmc_test_stress_perf,
	},
};
