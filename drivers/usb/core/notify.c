...
#if 0
#ifdef MBB_USB_CHARGE 
static BLOCKING_NOTIFIER_HEAD(usb_otg_event_list);
/**
 * usb_register_otg_notify - register a notifier callback whenever a usb otg change happens
 * @nb: pointer to the notifier block for the callback events.
 *
 * These changes are either USB ID have been connected to GND or disconnect.
 */
void usb_register_otg_notify(struct notifier_block *nb)
{
 ...
 * to work properly.
 */
void usb_unregister_otg_notify(struct notifier_block *nb)
{
