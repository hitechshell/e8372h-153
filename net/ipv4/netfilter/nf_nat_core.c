...
extern void ip_conntrack_ipct_add(struct sk_buff *skb, __u32 hooknum,
    struct nf_conn *ct, enum ip_conntrack_info ci, struct nf_conntrack_tuple *manip);
#endif

#if(FEATURE_ON == MBB_FEATURE_FASTIP)
extern void fastip_ip_conn_cachet_add(struct sk_buff *skb,__u32 hooknum,
	...
	hlist_for_each_entry_rcu(nat, n, &net->ipv4.nat_bysource[h], bysource) {
		...
		if (same_src(ct, tuple) && nf_ct_zone(ct) == zone) {
			...
			if (in_range(result, range)) {
				...
				return 1;
#else
				if(!(range->flags & NF_NAT_SYMMETRIC))
				{
                    return 1;
				}
				else
				{
					range->flags = range->flags | NF_NAT_RANGE_PROTO_RANDOM;
					return 0;
				}
#endif 
			}
...
#ifdef CONFIG_ATP_SYMNAT  
    struct nf_nat_ipv4_range *tmp_range = (struct nf_nat_ipv4_range *)range;
#endif
...
#if (FEATURE_ON == MBB_FEATURE_FASTIP)
        fastip_ip_conn_cachet_add(skb, hooknum, ct, ctinfo, &target);
#endif
