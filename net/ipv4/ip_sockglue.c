...
#ifdef CONFIG_PRSITE
int (*prsite_add_hook)(char __user * optval)  = NULL;
int (*prsite_del_hook)(char __user * optval)  = NULL;
int (*prsite_ipconflict_hook)(char __user * optval)  = NULL;
#if defined(BSP_CONFIG_BOARD_401HW) || defined(BSP_CONFIG_BOARD_506HW) \
 || defined(BSP_CONFIG_BOARD_506HW_2)
int (*ssid2_checkurl_del_hook)(char __user * optval)  = NULL;
#endif
#if defined(CONFIG_FORCE_APP)
int (*prsite_setmaclist_hook)(char __user * optval, unsigned int optlen)  = NULL;
#endif
#endif
...
mc_msf_out:
    ...
    case PRSITE_MACLIST:
    {
        if (prsite_setmaclist_hook)
        {
            if ((*prsite_setmaclist_hook)(optval, optlen))
            {
                release_sock(sk);
                return -EFAULT;
            }
...
#endif
    case PRSITE_IPCONFLICT:
    {
        if (prsite_ipconflict_hook)
        {
            if ((*prsite_ipconflict_hook)(optval))
            {
                release_sock(sk);
                return -EFAULT;
            }
...
#if defined(BSP_CONFIG_BOARD_401HW) || defined(BSP_CONFIG_BOARD_506HW) \
 || defined(BSP_CONFIG_BOARD_506HW_2)
    case SSID2_CHECKURL_DEL:
    {
        if (ssid2_checkurl_del_hook)
        {
            if ((*ssid2_checkurl_del_hook)(optval))
            {
                release_sock(sk);
                return -EFAULT;
            }
